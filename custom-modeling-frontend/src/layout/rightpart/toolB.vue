<template>
  <div class="container">
    <el-form ref="form" :model="item" label-width="80px" size="mini">
      ...
      <div>
        <el-radio v-model="item.custom_data.method" label="ASC" border>升序</el-radio>
        <el-radio v-model="item.custom_data.method" label="DESC" border>降序</el-radio>
      </div>
...
export default {
  ...
  methods: {
    ...
    flash() {
      ...
      if(this.topNode){
        this.$set(this.item.data_state,'field', this.topNode.data_state.field.filter(it => it.checked === "true") || []);
      }
      else{
    ...
    async handle_exec(){
      ...
      let payload = {
            ...
            table_header: this.topNode.data_state.field.filter(it => it.checked === "true"),
            table_data: res.data,          
        };
        this.openTable();
        this.countDataNum();
